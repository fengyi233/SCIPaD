...
class StuEnc(nn.Module):
    def __init__(self, cfg):
        super(StuEnc, self).__init__()
        self.set_missing_to_max = True
        min_depth_bin = 0.1
        max_depth_bin = 20.0
        self.num_ch_enc = np.array([64, 64, 128, 256, 512])
        self.num_depth_bins = cfg.stunet.num_depth_bins
        # we build the cost volume at 1/4 resolution
        self.matching_height, self.matching_width = cfg.height // 4, cfg.width // 4

        self.device = torch.device(f"cuda:{cfg.device}")
        self.warp_depths = None
        self.depth_bins = None

        encoder = getattr(models, cfg.stunet.backbone)(weights='DEFAULT')
        ...
        if int(cfg.stunet.backbone[6:]) > 34:
            self.num_ch_enc[1:] *= 4
    ...
    def compute_depth_bins(self, min_depth_bin, max_depth_bin):
        ...
        self.depth_bins = torch.linspace(min_depth_bin, max_depth_bin, self.num_depth_bins).float()

        self.warp_depths = torch.ones((self.num_depth_bins, self.matching_height, self.matching_width),
                                      dtype=torch.float32,
                                      device=self.device)
        self.warp_depths = self.warp_depths * self.depth_bins.to(self.device).unsqueeze(-1).unsqueeze(-1)
        self.warp_depths = self.warp_depths.unsqueeze(1)  # [num_bins, 1, h, w]
    ...
    def match_features(self, current_feats, lookup_feats, relative_poses, K, invK):
        ...
        pose, known intrinsics and using hypothesised depths self.warp_depths.
        ...
        for batch_idx in range(len(current_feats)):
            ...
            for lookup_idx in range(_lookup_feats.shape[1]):
                ...
                diffs = torch.abs(warped - current_feats[batch_idx:batch_idx + 1]).mean(1) * edge_mask  # [96, h, w]
    ...
    def forward(self, current_image, lookup_images, poses, K, invK,
        ...
        with torch.no_grad():
            ...
            lookup_feats = lookup_feats.reshape(batch_size, num_frames, chns, height, width)  # [b,n,c,h,w]
...
class TeaEnc(nn.Module):
    def __init__(self, cfg):
        super(TeaEnc, self).__init__()

        self.num_ch_enc = np.array([64, 64, 128, 256, 512])
        self.encoder = getattr(models, cfg.teanet.backbone)(weights='DEFAULT')
        if int(cfg.teanet.backbone[6:]) > 34:
            self.num_ch_enc[1:] *= 4
